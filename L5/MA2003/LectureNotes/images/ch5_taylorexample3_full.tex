...
\begin{tikzpicture}
\draw[pattern=north west lines,pattern color=gray!50,draw=white] (-1,-1.5) -- (-1,3) -- (4,3) -- (4,-1.5) -- (-1,-1.5);
\draw[->,thick,gray] (-1,0) -- (4,0);% node[right]{$\mathrm{Re}$};
\draw[->,thick,gray] (0,-1.5) -- (0,3);% node[above]{$\mathrm{Im}$};
\draw[fill] (2,1) circle (2pt) node[below,font=\footnotesize]{$2+i$};
\draw[fill=white] (0,-1) circle (2pt) (0,1) circle (2pt) ;
\draw[dashed,thick,pattern=north east lines,pattern color=gray!80] (2,1) circle (1.95);
\draw (2,-1.2) node[font=\footnotesize]{$D(2+i,2)$};
