...
\begin{tikzpicture}
\draw[pattern=north west lines,pattern color=gray!50,draw=white] (-2,-2) -- (-2,2) -- (2,2) -- (2,-2) -- (-2,-2);
\draw[->,thick,gray] (-2,0) -- (1.75,0) node[right]{$\mathrm{Re}$};
\draw[->,thick,gray] (0,-2) -- (0,1.75) node[above]{$\mathrm{Im}$};
\draw[fill=white] (0,0) circle (2pt);
\draw[thick,fill] (-1,1) circle (2pt) node[above]{$z_*$} -- (1,-1) circle(2pt) node[below]{$-z_*$} [arrow inside={end=stealth,opt={red,scale=1.5}}{0.7}];
